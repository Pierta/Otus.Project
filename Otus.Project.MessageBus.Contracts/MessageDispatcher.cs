...
namespace Otus.Project.MessageBus.Contracts
{
    public class MessageDispatcher : IAutoSubscriberMessageDispatcher
    {
        ...
        public void Dispatch<TMessage, TConsumer>(TMessage message, CancellationToken ct)
            where TMessage : class
            where TConsumer : class, IConsume<TMessage>
        {
            using var scope = provider.CreateScope();
            var consumer = scope.ServiceProvider.GetRequiredService<TConsumer>();
            consumer.Consume(message, ct);
        }

        public async Task DispatchAsync<TMessage, TConsumer>(TMessage message, CancellationToken ct)
            where TMessage : class
